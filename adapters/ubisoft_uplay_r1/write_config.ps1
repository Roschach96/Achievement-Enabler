# adapters\ubisoft_uplay_r1\write_config.ps1
#
...
#   AE_ADAPTER_DIR   - full path to adapters\ubisoft_uplay_r1 (this folder)
#   AE_ASSETS_DIR    - full path to the UplayR1-* assets folder
...
if ($missing.Count -gt 0) {
    Write-Host "[ERROR] ubisoft_uplay_r1\write_config.ps1: missing env var(s): $($missing -join ', ')"
    exit 1
}

# ── Step 0: failsafe - the UplayR1-* asset pack must be complete ───
# Ships inside adapters\ubisoft_uplay_r1\UplayR1-*. If it's missing entirely,
...
if (-not $assetsDir -or -not (Test-Path -LiteralPath $assetsDir)) {
    Write-Host "[ERROR] Uplay R1 asset pack (UplayR1-*) was not found under: $adapterDir"
    Write-Host "[ERROR] Please redownload the latest Uplay R1 emulator files and place the"
    Write-Host "[ERROR] 'UplayR1-*' folder inside adapters\ubisoft_uplay_r1\."
    exit 1
}

$missingAssets = $requiredAssetFiles | Where-Object { -not (Test-Path -LiteralPath (Join-Path $assetsDir $_)) }
if ($missingAssets.Count -gt 0) {
    Write-Host "[ERROR] The Uplay R1 asset pack at '$assetsDir' is missing required file(s):"
    foreach ($f in $missingAssets) { Write-Host "  - $f" }
    Write-Host "[ERROR] Please redownload the latest Uplay R1 emulator files and replace the"
    Write-Host "[ERROR] 'UplayR1-*' folder inside adapters\ubisoft_uplay_r1\ with the complete version."
    exit 1
}

# ── Step 1: back up any pre-existing loader DLLs / INIs in place ──────────
$backupTargets = @('upc_r1_loader.dll', 'upc_r1_loader64.dll', 'uplay_r1_loader.dll', 'uplay_r1_loader64.dll')
...
foreach ($name in @('upc_r1.ini', 'uplay_r1.ini')) {
    $p = Join-Path $destination $name
    $bak = "$p.BAK"
    if (-not (Test-Path -LiteralPath $bak) -and (Test-Path -LiteralPath $p)) {
        Rename-Item -LiteralPath $p -NewName "$name.BAK"
...
$matchedIni = switch -Regex ($dllName) {
    '(?i)^upc_r1_loader(64)?\.dll$'   { 'upc_r1.ini';   break }
    '(?i)^uplay_r1_loader(64)?\.dll$' { 'uplay_r1.ini'; break }
    default { $null }
}
...
if ($matchedIni) {
    ...
    $filesToCopy = @('upc_r1.ini', 'upc_r1_loader.dll', 'upc_r1_loader64.dll', 'uplay_r1.ini', 'uplay_r1_loader.dll', 'uplay_r1_loader64.dll')
}
...
foreach ($iniName in @('upc_r1.ini', 'uplay_r1.ini')) {
