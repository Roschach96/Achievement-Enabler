# adapters\ea_origin_emulator\make_shortcut.ps1
...
#
# Unlike steam_coldclient, there's no separate loader exe to point at - the
# shortcut targets the game's own exe directly, with anadius.cfg sitting
# next to it in the game folder (that's how anadius.cfg's own model works;
# there's nothing to "launch through").
#
# Reads env vars set by AchievementEnabler.bat:
#   AE_EXE_PATH   - full path to the selected game exe
#   AE_GAME_NAME  - the game's own top-level folder name
...
# AE_GAME_NAME (the game's own top-level folder name, set once by the
# orchestrator) rather than deriving a name from the exe's own parent
# folder: steam_coldclient can safely do that because its exe always sits
# at a fixed, known depth (<gameFolder>\_ColdClient), but this adapter's
# exe can be nested arbitrarily deep (e.g. 00_game\target_origin\ex\
# WILD HEARTS.exe) - taking the exe's immediate parent folder name there
# would give "ex", not the game's name.
$name = if ($env:AE_GAME_NAME) { $env:AE_GAME_NAME } else { Split-Path -Leaf $work }

function Try-Shortcut($lnkPath, $shortcutName) {
    try {
        $ok = [AchievementEnabler.ShortcutHelper]::Create($lnkPath, $exePath, $work, $icon, 0)
        if ($ok) {
