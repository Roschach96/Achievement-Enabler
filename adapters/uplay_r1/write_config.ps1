# adapters\uplay_r1\write_config.ps1
#
...
#   AE_ADAPTER_DIR   - full path to adapters\uplay_r1 (this folder)
...
if ($missing.Count -gt 0) {
    Write-Host "[ERROR] uplay_r1\write_config.ps1: missing env var(s): $($missing -join ', ')"
    exit 1
}

# ── Step 0: failsafe - the UplayR1-* asset pack must be complete ───
# Ships inside adapters\uplay_r1\UplayR1-*. If it's missing entirely,
...
if (-not $assetsDir -or -not (Test-Path -LiteralPath $assetsDir)) {
    ...
    Write-Host "[ERROR] 'UplayR1-*' folder inside adapters\uplay_r1\."
    exit 1
}
...
if ($missingAssets.Count -gt 0) {
    ...
    Write-Host "[ERROR] 'UplayR1-*' folder inside adapters\uplay_r1\ with the complete version."
    exit 1
}
