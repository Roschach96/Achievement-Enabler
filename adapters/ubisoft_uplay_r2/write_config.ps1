# adapters\ubisoft_uplay_r2\write_config.ps1
#
...
#   AE_ADAPTER_DIR   - full path to adapters\ubisoft_uplay_r2 (this folder)
...
if ($missing.Count -gt 0) {
    Write-Host "[ERROR] ubisoft_uplay_r2\write_config.ps1: missing env var(s): $($missing -join ', ')"
    exit 1
}

# ── Step 0: failsafe - the GoldbergUplayR2-* asset pack must be complete ───
# Ships inside adapters\ubisoft_uplay_r2\GoldbergUplayR2-*. If it's missing entirely,
...
if (-not $assetsDir -or -not (Test-Path -LiteralPath $assetsDir)) {
    ...
    Write-Host "[ERROR] 'GoldbergUplayR2-*' folder inside adapters\ubisoft_uplay_r2\."
    exit 1
}
...
if ($missingAssets.Count -gt 0) {
    ...
    Write-Host "[ERROR] 'GoldbergUplayR2-*' folder inside adapters\ubisoft_uplay_r2\ with the complete version."
    exit 1
}
